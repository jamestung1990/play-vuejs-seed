define(function(require) {
  ...
  function timeEntries(bus) {
    return net.asyncGet('/sys-template/timeEntries').then(function(_template) {
  ...
  // emitted...
  function app(bus) {
    return net.asyncGet('/sys-template/app').then(function(_template) {
      var data = {
        navKey : 'home',
        totalTime : 0.0
      };
  ...
  Promise.all([ navbar(bus), sidebar(bus), logTime(bus), timeEntries(bus), home(bus), app(bus) ]).then(
